# AWS Certified Cloud Practitioner — Practice Test #5 中英對照
...
* Close coupling／緊耦合
* Servers, not services／伺服器，而非服務
* Security comes last／安全最後考量
* Automation／自動化
...
* Data type／資料類型
* The Amazon EC2 instance type the EBS volume is attached to／附加的 EC2 執行個體類型
* Data transfer IN／資料傳入
* Volume type／磁碟區類型
* Provisioned IOPS／佈建的 IOPS
...
* Enable traceability／啟用可追溯性
* Automatically recover from failure／自動從故障中恢復
* Use serverless architectures／使用無伺服器架構
* Anticipate failure／預測故障
* Make frequent, small, reversible changes／進行頻繁、小型、可回復的變更
...
* A virtual private gateway (VGW)／虛擬私有閘道
* A VPC endpoint interface／VPC 端點介面
...
* Automate security assessments／自動化安全評估
* Analyze against unintended network accessibility／分析非預期的網路可存取性
* Inspect running OS against known vulnerabilities／檢查執行中的作業系統是否存在已知漏洞
...
* Enforcing secure communication between VPCs using network traffic filtering／使用網路流量過濾強制執行 VPC 之間的安全通訊
* Protecting web applications from common exploits such as SQL injection／保護 Web 應用程式免受 SQL 注入等常見攻擊
* Encrypting data in transit between AWS services using TLS certificates／使用 TLS 憑證加密 AWS 服務之間的傳輸資料
...
* Electronic equipment at office／辦公室電子設備
* Number of end-users／終端使用者數量
* Application advertising／應用程式廣告
* Server administration／伺服器管理
* Power/Cooling／電力／冷卻
...
* Load balancing／負載平衡
* IP routing／IP 路由
* Transfer acceleration／傳輸加速
* Domain registration／網域註冊
* Health checks and monitoring／健康檢查與監控
...
* Standard reserved instance (RI)／標準保留執行個體
* Scheduled reserved instance (RI)／排程保留執行個體
...
* Data center security／資料中心安全
* Customer data／客戶資料
* Disposal of disk drives／磁碟機的處置
* Operating system (OS) configuration／作業系統設定
...
* Installing security patches of the guest operating system (OS)／安裝客體作業系統的安全修補程式
* Encrypting application data／加密應用程式資料
* Configuring IAM Roles／設定 IAM 角色
* Network operability／網路可操作性
* Data center security／資料中心安全
...
* As employees travel, they can use other employees' accounts／員工出差時可使用其他員工的帳戶
* Create an IAM user for each user in each AWS region／在每個 AWS 區域為每位使用者建立 IAM 使用者
* Create global permissions so users can access resources from all around the world／建立全域權限使使用者可存取全球資源
...
* Use AWS Cost Explorer to view and track your usage in detail／使用 AWS Cost Explorer 詳細檢視和追蹤使用情況
* Use AWS CloudFormation to automate security best practices／使用 AWS CloudFormation 自動化安全最佳實務
* Use Amazon CloudWatch to measure overall efficiency／使用 Amazon CloudWatch 衡量整體效率
...
* Distributing load to Amazon EC2 instances／將負載分配至 EC2 執行個體
* Lower cost by adjusting the number of EC2 instances／透過調整 EC2 執行個體數量降低成本
* Having the right amount of computing capacity／擁有適當的運算容量
...
* Amazon S3 Bucket policies／S3 儲存貯體政策
* S3 Cross-Region Replication (S3 CRR)／S3 跨區域複寫
...
* A VPC spans all AWS regions within an Availability Zone (AZ)／VPC 跨越一個 AZ 內的所有 AWS 區域
* A VPC spans all Availability Zones (AZs) in all AWS regions／VPC 跨越所有 AWS 區域的所有 AZ
* Amazon VPC spans all subnets in all AWS regions／Amazon VPC 跨越所有 AWS 區域的所有子網路
...
* Amazon EBS Elastic Volumes are bound to a specific AZ／EBS 彈性磁碟區綁定至特定 AZ
* Amazon EBS Elastic Volumes can be mounted to one instance at a time／EBS 彈性磁碟區一次只能掛載至一個執行個體
* Amazon EBS Elastic Volumes can persist data after their termination／EBS 彈性磁碟區可在終止後保留資料
...
* IAM credentials report／IAM 憑證報告
...
* Opt for a higher AWS Support plan／選擇更高的 AWS Support 方案
* Vertically scale the EC2 instances／垂直擴展 EC2 執行個體
* Build its own servers／自建伺服器
* Set up Auto Scaling groups to align the number of instances with the demand／設定 Auto Scaling 群組以使執行個體數量符合需求
* Purchase Amazon EC2 Reserved instances (RIs)／購買 Amazon EC2 保留執行個體
...
* Disable AWS CloudTrail on several accounts／在部分帳戶上停用 AWS CloudTrail
* Never use tags for billing／從不使用標籤進行計費
* Do not use AWS Organizations to automate AWS account creation／不使用 AWS Organizations 自動建立 AWS 帳戶
* Restrict account privileges using Service Control Policies (SCP)／使用 SCP 限制帳戶權限
* Create AWS accounts per department／按部門建立 AWS 帳戶
...
* Limited scaling／有限的擴展性
* AWS is responsible for security in the cloud／AWS 負責雲端中的安全
* Trade operational expense for capital expense／以資本支出換取營運支出
* Stop guessing about capacity／停止猜測容量需求
...
* Protecting hardware infrastructure／保護硬體基礎設施
* Edge locations security／邊緣位置安全
* Managing Amazon DynamoDB／管理 Amazon DynamoDB
...
* API access／API 存取
* Performance and availability of AWS services／AWS 服務的效能和可用性
* Account management／帳戶管理
* Application performance／應用程式效能
...
* Managing increasing volumes of data／管理不斷增加的資料量
* Loose coupling／鬆耦合
...
* It is a PaaS that allows you to model and provision resources needed for an application／它是一種 PaaS，允許您為應用程式建模和佈建所需資源
* It is an IaC that allows you to model and provision resources needed for an application／它是一種 IaC，允許您為應用程式建模和佈建所需資源
* It is an IaaS that allows you to deploy and scale web applications and services／它是一種 IaaS，允許您部署和擴展 Web 應用程式和服務
...
* Create one account for all departments and share this account／為所有部門建立一個帳戶並共用
* Create different VPCs for different departments／為不同部門建立不同的 VPC
* Create different accounts for different departments／為不同部門建立不同帳戶
