# AWS Certified Cloud Practitioner — Practice Test #2 中英對照
...
一家電子商務公司希望評估其在 Amazon EC2 執行個體上部署的應用程式是否存在漏洞以及與 AWS 最佳實務的偏差。哪個 AWS 服務可以用於促進此任務？
...
一家社群媒體公司希望保護其 Web 應用程式免受 SQL 注入和跨站腳本等常見 Web 攻擊。以下哪個 AWS 服務可以用於解決此使用案例？
...
* Amazon EC2 instances, Amazon EC2 Auto Scaling groups／Amazon EC2 執行個體、Amazon EC2 Auto Scaling 群組
* Amazon Elastic Block Store (Amazon EBS), AWS Lambda functions／Amazon EBS、AWS Lambda 函數
...
一家 IT 公司希望每週一凌晨 2 點執行一次日誌備份流程，通常執行時間為 5 分鐘。您會推薦哪些 AWS 服務來為此使用案例建立無伺服器解決方案？（選兩項）
...
* Availability Zone (AZ) infrastructure management／可用區域（AZ）基礎設施管理
* Configuration management for AWS global infrastructure／AWS 全球基礎設施的設定管理
* Patching/fixing flaws within the AWS infrastructure／修補/修復 AWS 基礎設施中的缺陷
* Managing patches of the guest operating system on Amazon EC2／管理 Amazon EC2 上客體作業系統的修補程式
...
哪個服務提供您雲端架構中 AWS 服務狀態的個人化視圖，以便在 AWS 服務出現問題時快速評估對您業務的影響？

* Amazon CloudWatch
* Amazon Inspector
* AWS Health - Service Health Dashboard／AWS Health - 服務健康儀表板
...
分散在不同可用區域（AZ）的 Amazon EC2 執行個體群需要存取、編輯和共享集中儲存在系統上的檔案型資料。您會推薦哪個 AWS 服務？

* EC2 Instance Store／EC2 執行個體存放區
* Amazon Simple Storage Service (Amazon S3)
...
* Customer Data／客戶資料
* Server-side Encryption (SSE)／伺服器端加密
...
* Modify a Database instance to higher CPU and RAM／將資料庫執行個體修改為更高的 CPU 和 RAM
* Modify an EC2 instance type from t2.nano to u-12tb1.metal／將 EC2 執行個體類型從 t2.nano 修改為 u-12tb1.metal
* Add a bigger CPU to a computer／為電腦新增更大的 CPU
* Elastic Load Balancing (ELB)
* Read Replicas in Amazon Relational Database Service (Amazon RDS)／Amazon RDS 中的讀取副本
...
* You can scale out and add more EC2 instances to match an increase in demand as well as scale in and remove EC2 instances to match a reduced demand／您可以向外擴展並新增更多 EC2 執行個體以符合需求增加，也可以縮減並移除 EC2 執行個體以符合需求減少
* You can automatically remove unhealthy instances／您可以自動移除不健康的執行個體
* You can automatically register new instances to a load balancer／您可以自動將新執行個體註冊至負載平衡器
* You can automatically deploy AWS Shield when a DDoS attack is detected／當偵測到 DDoS 攻擊時可以自動部署 AWS Shield
...
* Detect text in a photo／偵測照片中的文字
* Identify person in a photo／識別照片中的人物
* Label objects in a photo／為照片中的物件加上標籤
...
一個組織計劃將其基礎設施從本地資料中心遷移至 AWS 雲端。您會推薦哪些選項，以幫助組織識別在 AWS 雲端上建立解決方案的正確 AWS 服務？（選兩項）
...
* AWS Lambda provides access to the underlying operating system to control its behavior through code／AWS Lambda 提供對底層作業系統的存取，以透過程式碼控制其行為
* AWS Lambda allows you to install databases on the underlying serverless Operating System／AWS Lambda 允許您在底層無伺服器作業系統上安裝資料庫
* AWS Lambda allows you to orchestrate and manage Docker containers to facilitate complex containerized applications on AWS／AWS Lambda 允許您編排和管理 Docker 容器以促進 AWS 上的複雜容器化應用程式
* AWS Lambda lets you run code without provisioning or managing servers／AWS Lambda 讓您無需佈建或管理伺服器即可執行程式碼
* You pay for the compute time you consume for AWS Lambda／您為 AWS Lambda 消耗的運算時間付費
...
哪個工具可以幫助您根據當前 AWS 最佳實務審查您的工作負載，包括成本優化、安全性和效能改善，然後獲得更好架構設計的建議？
...
* Amazon RDS Multi-AZ reduces database usage costs／Amazon RDS Multi-AZ 降低資料庫使用成本
* Amazon RDS Multi-AZ improves database performance for read-heavy workloads／Amazon RDS Multi-AZ 改善讀取密集型工作負載的資料庫效能
* Amazon RDS Multi-AZ protects the database from a regional failure／Amazon RDS Multi-AZ 保護資料庫免受區域故障影響
* Amazon RDS Multi-AZ enhances database availability／Amazon RDS Multi-AZ 提升資料庫可用性
...
一家遊戲公司正在尋找一種技術/服務，可以提供一致的低延遲遊戲體驗，以確保各地終端使用者獲得良好的使用者體驗。哪個 AWS 技術/服務將為終端使用者提供必要的低延遲存取？

* AWS Wavelength
* AWS Edge Locations／AWS 邊緣位置
* AWS Direct Connect
* AWS Local Zones／AWS 本地區域
...
一家公司希望為其 Windows 型應用程式提供全受管、靈活且可擴展的檔案儲存系統，且具有低延遲存取。哪個 AWS 服務是正確的選擇？
...
AWS 雲端採用框架（AWS CAF）的哪個雲端轉型旅程階段專注於展示雲端如何幫助加速您的業務成果？
...
* Application portfolio management／應用程式組合管理
...
* Neither inbound nor outbound data transfer are charged／進出站資料傳輸均不收費
* Only inbound data transfer is charged／只有入站資料傳輸收費
* Both inbound data transfer and outbound data transfer are charged／入站和出站資料傳輸均收費
...
* Root user credentials should only be shared with managers requiring administrative responsibilities to complete their jobs／根使用者憑證應僅與需要管理職責的管理員共享
* Root user account gets unrestricted permissions when the account is created, but these can be restricted using IAM policies／建立帳戶時根使用者帳戶獲得不受限制的權限，但可以使用 IAM 政策限制這些權限
* Root user account password cannot be changed once it is set／根使用者帳戶密碼一旦設定就無法更改
* It is highly recommended to enable Multi-Factor Authentication (MFA) for root user account／強烈建議為根使用者帳戶啟用多重要素驗證（MFA）
* Root user access credentials are the email address and password used to create the AWS account／根使用者存取憑證是用於建立 AWS 帳戶的電子郵件地址和密碼
...
* Use Amazon S3 Standard to store the thumbnails／使用 Amazon S3 Standard 儲存縮圖
* Use Amazon S3 Standard-Infrequent Access (S3 Standard-IA) to store the thumbnails／使用 Amazon S3 Standard-IA 儲存縮圖
* Use Amazon S3 Glacier Flexible Retrieval to store the thumbnails／使用 Amazon S3 Glacier Flexible Retrieval 儲存縮圖
* Use Amazon S3 One Zone-Infrequent Access (S3 One Zone-IA) to store the thumbnails／使用 Amazon S3 One Zone-IA 儲存縮圖
...
一家電子商務公司希望將推薦引擎的資料儲存在資料庫中。您會推薦哪個 AWS 服務以在任何規模下提供最少的營運負擔？
...
* Applying patches to the Amazon RDS database／為 Amazon RDS 資料庫套用修補程式
* Applying patches to the underlying OS／為底層作業系統套用修補程式
* Managing the underlying server hardware on which Amazon RDS runs／管理 Amazon RDS 執行所在的底層伺服器硬體
...
一家公司的旗艦應用程式在 Amazon EC2 執行個體群上執行。系統管理員正在尋找最佳方式，在不開啟新連接埠或使用公用 IP 位址的情況下提供對 Amazon EC2 執行個體的安全 Shell 存取。哪個工具/服務可以幫助您實現此需求？
...
* Amazon S3 is file based storage, Amazon EBS is block based storage and Amazon EFS is object based storage／Amazon S3 是檔案型儲存，Amazon EBS 是區塊型儲存，Amazon EFS 是物件型儲存
* Amazon S3 is block based storage, Amazon EBS is object based storage and Amazon EFS is file based storage／Amazon S3 是區塊型儲存，Amazon EBS 是物件型儲存，Amazon EFS 是檔案型儲存
* Amazon S3 is object based storage, Amazon EBS is file based storage and Amazon EFS is block based storage／Amazon S3 是物件型儲存，Amazon EBS 是檔案型儲存，Amazon EFS 是區塊型儲存
* Amazon S3 is object based storage, Amazon EBS is block based storage and Amazon EFS is file based storage／Amazon S3 是物件型儲存，Amazon EBS 是區塊型儲存，Amazon EFS 是檔案型儲存
...
* Amazon CloudWatch
* AWS CloudFormation
* AWS Health Dashboard - service health／AWS Health Dashboard - 服務健康狀態
...
* Compute, Databases and Outbound Data Transfer／運算、資料庫和出站資料傳輸
* Compute, Storage and Inbound Data Transfer／運算、儲存和入站資料傳輸
* Compute, Databases and Inbound Data Transfer／運算、資料庫和入站資料傳輸
...
* Both the VPC and the subnet span only one AZ in the Region／VPC 和子網路都僅跨越區域中的一個 AZ
* A subnet spans all of the AZs in the Region whereas a VPC spans only one AZ in the Region／子網路跨越區域中所有 AZ，而 VPC 僅跨越一個 AZ
* Both the VPC and the subnet span all of the AZs in the Region／VPC 和子網路都跨越區域中所有 AZ
* A VPC spans all of the AZs in the Region whereas a subnet spans only one AZ in the Region／VPC 跨越區域中所有 AZ，而子網路僅跨越一個 AZ
...
* AWS Lambda
* Amazon Elastic Container Service (Amazon ECS) - EC2 launch type／Amazon ECS - EC2 啟動類型
* Amazon Elastic Compute Cloud (Amazon EC2)
* Amazon Elastic Container Service (Amazon ECS) - Fargate launch type／Amazon ECS - Fargate 啟動類型

---

## Q57
Which of the following statement is correct for a Security Group and a Network Access Control List (Network ACL)?
以下哪項陳述關於安全群組和網路存取控制清單（網路 ACL）是正確的？

* Security Group acts as a firewall at the subnet level whereas Network ACL acts as a firewall at the instance level／安全群組在子網路層級充當防火牆，而網路 ACL 在執行個體層級充當防火牆
* Security Group acts as a firewall at the VPC level whereas Network ACL acts as a firewall at the AZ level／安全群組在 VPC 層級充當防火牆，而網路 ACL 在 AZ 層級充當防火牆
* Security Group acts as a firewall at the AZ level whereas Network ACL acts as a firewall at the VPC level／安全群組在 AZ 層級充當防火牆，而網路 ACL 在 VPC 層級充當防火牆
* Security Group acts as a firewall at the instance level whereas Network ACL acts as a firewall at the subnet level／安全群組在執行個體層級充當防火牆，而網路 ACL 在子網路層級充當防火牆
...
* AWS Trusted Advisor
* AWS Applicable Use Policy／AWS 適用使用政策
* AWS Fair Use Policy／AWS 公平使用政策
...
一家資料分析公司將其資料儲存在 Amazon S3 上，希望以最少的工作量對此資料進行基於 SQL 的分析。您會建議哪個 AWS 服務用於此使用案例？
...
* Tape Gateway, Object Gateway and Volume Gateway／磁帶閘道、物件閘道和磁碟區閘道
* Tape Gateway, File Gateway and Block Gateway／磁帶閘道、檔案閘道和區塊閘道
* Object Gateway, File Gateway and Block Gateway／物件閘道、檔案閘道和區塊閘道
